...
const DentistLogin = ({ setDentist, setUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5001/api/dentists/login",
        form
      );
      localStorage.setItem("token", res.data.token);
      setDentist(res.data.dentist);
  ...
  return (
    <div className="max-w-md mx-auto p-4">
      ...
      <h2 className="text-2xl mb-4 dark:text-white flex items-center justify-center">
        <FaSignInAlt className="mr-2" /> Dentist Login{" "}
      ...
      <p className="mt-4 text-center dark:text-white">
        ...
        <button
          onClick={() => navigate("/dentist-register")}
          className="text-blue-500 hover:underline flex items-center justify-center mx-auto">
          <FaUserPlus className="mr-1" /> Register as a Dentist
        </button>
      </p>
    </div>
  );
